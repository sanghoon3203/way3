...
**프로젝트명**: connect:seoul (구 WAY3)
**플랫폼**: iOS (SwiftUI)
**장르**: 위치 기반 GPS 트레이딩 RPG + Visual Novel
**배경**: 서울시 25개 구 (네오 서울 세계관)
**최종 업데이트**: 2026-02-20
...
connect:seoul은 실제 서울을 무대로 한 위치 기반 트레이딩 RPG입니다.
...
- **AI 자유대화**: 상인별 Gemini AI 인격 — 일상 대화 + 에피소드 언락 트리거
- **오프라인 우선**: 스토리·퀘스트 데이터는 앱 번들 내 JSON으로 로컬 운영
- **조선사이버펑크**: ChosunCentennial + Pretendard 폰트, 오방색 네온 팔레트
...
│       ├── images/            ← 배경 이미지
│       ├── Sound/             ← BGM/SFX
│       └── font/              ← ChosunCentennial_otf.otf + Pretendard-*.otf
│
├── claudedocs/                ← 기획/설계 문서
├── docs/plans/                ← UI/시스템 설계 문서
└── Story/                     ← 내러티브 기획 원본
...
Gemini API 기반 캐릭터 자유대화. `claudedocs/05_AI_CHAT_SYSTEM.md` 참조.

- 상인별 시스템 프롬프트 (성격/말투/세계관 제약)
- 키워드 감지 → VN 에피소드 언락 트리거
- 대화창: MerchantDetailView `.chat` 탭
...
| 상인 | 구역 | GPS 좌표 | 서브스토리 | AI 인격 |
|------|------|---------|-----------|--------|
| 서예나 | 강남구 — 압구정 로데오 | 37.5279, 127.0388 | ✅ 152노드 | ✅ |
| 앨리스강 | 서초구 — 서래마을 | 37.4914, 127.0032 | ⚠️ 59노드 | ✅ |
| 애니박 | 송파구 — 잠실 롯데월드 | 37.5111, 127.0982 | ⚠️ 66노드 | ✅ |
| 진백호 | 강동구 — 천호동 | 37.5402, 127.1236 | ⚠️ 26노드 | ✅ |
| 주블수 | 강동구 — 천호동 | 37.5407, 127.1241 | ⚠️ 20노드 | ✅ |
| 김세휘 | 관악구 — 서울대 | 37.4603, 126.9517 | ❌ 미작성 | ✅ |
| 기주리 | 종로구 — 경복궁 | 37.5759, 126.9768 | ❌ 미작성 | ✅ |
| 카타리나 최 | 중구 — 명동성당 | 37.5636, 126.9868 | ❌ 미작성 | ✅ |
| 마리 | 마포구 — 홍대 | 37.5487, 126.9220 | ❌ 미작성 | ✅ |

---

## 서버 아키텍처

**기술 스택**: Node.js 18+ / Express.js / SQLite3 / JWT
**배포**: Railway.app
...
/api/merchant-chat          ← ✨NEW: AI 상인 채팅
  POST /               — { merchantId, message, history } → { reply, unlockedEpisode }

/api/trade
  POST /execute
  GET  /history, /market-prices
...
- CORS, Helmet 보안 헤더
- Gemini API Key: 서버 측 환경 변수 (`GEMINI_API_KEY`) — 클라이언트에 노출 없음
...
cd way-server
npm install
cp .env.example .env  # 환경변수 설정 (GEMINI_API_KEY 포함)
npm run migrate
npm run seed
npm run dev
```
...
- 인증 시스템 (로그인, 회원가입, JWT)
- 플레이어 프로필 시스템
- 조선사이버펑크 UI 시스템 (오방색 팔레트, ChosunCentennial + Pretendard 폰트)
...
- AI 상인 채팅 시스템 (Gemini API, 9명 인격 정의)
...
| 문서 | 설명 |
|------|------|
| `claudedocs/00_INDEX.md` | 전체 문서 네비게이션 |
| `claudedocs/02_FIELD_GUIDE.md` | iOS 런타임 아키텍처 + 코드 레벨 가이드 |
| `claudedocs/03_GAME_SYSTEM.md` | VNNode/퀘스트 JSON 스키마 |
| `claudedocs/04_UI_SYSTEM.md` | 조선사이버펑크 디자인 시스템 |
| `claudedocs/05_AI_CHAT_SYSTEM.md` | AI 상인 채팅 시스템 |
| `claudedocs/status/STORY_STATUS.md` | 스토리 데이터 완성도 현황 |
| `claudedocs/status/QUEST_STATUS.md` | 퀘스트 데이터 현황 |
| `docs/plans/2026-02-20-ai-merchant-chat-design.md` | AI 채팅 설계 문서 |
