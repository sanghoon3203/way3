...
struct PlayerRowView: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            ZStack {
                Circle()
                    .fill(Color.gameBlue.opacity(0.2))
            ...
            Button(action: onTradeOffer) {
                Image(systemName: "handshake.fill")
                    ...
                    .background(Circle().fill(Color.gameGreen))
            }
        }
        .padding(.vertical, 4)
    }
                                    ...
                                    .overlay(
                                        RoundedRectangle(cornerRadius: 8)
                                            .stroke(selectedOfferedItems.contains(item) ? 
                                                   Color.gameGreen : Color.gray.opacity(0.3), 
                                                   lineWidth: 1)
                                    )
                            )
                            .foregroundColor(selectedOfferedItems.contains(item) ? 
                                           Color.gameGreen : Color.primary)
                    }
                }
            }
    ...
    private var sendOfferButton: some View {
        Button(action: sendTradeOffer) {
            Text("거래 제안 보내기")
                ...
                .background(
                    RoundedRectangle(cornerRadius: 12)
                        .fill(canSendOffer ? Color.gameGreen : Color.gray)
                )
        }
